...
std::string Join::execute(std::string command, std::string clientName)
{
	...
	std::cout<< "estoy imprimiendo el nombre dentro del execute de JOIN: " << channelName << std::endl;
	posCliente = 0;
	...
	if(posChannels == data->getChannels().size())
	{
		...
		std::cout << "imprimo en Join mediante data get channels el nombre del canal "<< data->getChannels()[data->getChannels().size() - 1].getName() << std::endl;
	}
